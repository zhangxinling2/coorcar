...
同样的先获得accountID，之后调用GetTrips，这里得到错误和获得行程不一样，这里出错是比较严重的错误，这里使用Logger打印出来并返回InternalError。然后把获得的Trips封装为Response。
...
定义一个calcCurrentStatus(trip *rentalpb.TripStatus,cur *rentalpb.Location)*rentalpb.LocationStatus来计算当前的状态
...
GetTrip出错则返回NotFound。如果当前位置的Location为nil，则当前位置就是数据库中的位置，先把trip的位置赋值给cur在去判断Current的Location是否为nil。由于获取Current很频繁，在前面先判断一下Current是否为nil，没有就直接返回错误。因为计算都是根据Current来的，没有的话就是数据坏掉了。

calcCurrentStatus返回的钱为上次的加上时间，由于现在是后端计算钱，所以LocationStatus要把时间timestamp_sec也带上。这里不要用UpdatedAt,那是为了乐观锁而添加的，它不属于后端计算价格的上下文，它是属于数据库的内部实现。领域开发要明确界限。由于时间使用了int64,生成文件时ts中Long类型报错，生成时加上参数--force-number强制为number。距离的话很复杂，这里直接做假数据，每秒钟跑0.02公里。

这里写完calcCurrentStatus，创建行程中的生成LocationStatus的逻辑就可以去直接调用calc方法。
...
这样可以把ProfileManager，CarManager外部传入，而不是内部定死。

### 身份信息的CRUD

![image-20240304215121619](C:\Users\K9\OneDrive\图片\go笔记\coolcar\状态改变流程.png)

两个小流程，上传图片不影响审查状态，因为图片相当于一个字段，只不过是特殊的字段。

#### 行程服务

proto中定义ProfileService，定义Identiy包含表格中的四个字段，lic_number,name,gender,birth_data_millis。定义Profile，它包含Indetity，它走动的状态要存在Profile里因为这个流程中**关心的是状态的转化不关心身份信息的实际值**，状态上有很多的业务逻辑。

##### Profile到底是不是实体？

如果是值类型就会维护一个account_id->Identity的映射关系。

如果是实体，他就会有生命周期。

初始获取Profile时，如果是值类型返回空Profile。因为值类型一直有。

如果是实体就会报错。

所以这里倾向于空Profile，返回空UI同样可以渲染。所以做成值类型。

状态定义中不需要is_not_specified,因为返回空Profile时这里的值就是0，空Profile处于一个UnSubmitted的状态。跟Gender不同，性别没填过就是不知道。

##### 服务定义

GetProfile,参数中Request中什么都没有，因为我只能获取自己的Profile，返回一个Profile。

SubmitProfile,参数就是Identity，返回Profile。

ClearProfile,Request中同样为空，返回Profile。

##### 数据库

ProfileRecord定义映射，ProfileRecord中包含AccountID和Profile

服务只需要GetProfile和UpdatedProfile

###### GetProfile

根据传入的accountId来找记录，然后返回*rentalpb.Profile

###### UpdateProfile

根据传入的accountid与profile来修改。服务层的submit会把identity拼成profile,clear会做一个清空的profile。

为accountId创建唯一索引。

Updated的索引条件为accountid和state，只有Unsubmmited状态时才可以Update，如果已经有记录那么因为创建了accountId的唯一索引，那么UpSert会挂掉。
...
GetProfile:得到accountId后就用数据库层的GetProfile去获取数据。

SubmitProfile:同样是先得到accountID，然后把Profile拼接出来，状态为pending，然后使用Update。必须是Unsubmitted条件下才能submit。

如果是修改前先get，那么就要和之前一样加updateat做乐观锁。

修改一下Update的传入参数，新增一个IdentityStatus前置条件。

ClearProfile:基本与Submit相同，只是Profile是空的。

为了不添加方法，把Clear实现为只有已经确认的profile才可以删除。
...
这样一个大的rental服务中就注册了trip和profile两个服务

##### gateway

同样的需要在localhost:8082中添加profile这个配置

##### 测试

测试身份认证的状态转换，所以在case中直接使用方法强行把它转换成verified。
...
register.ts中onload中要先拿profile，由于后面可能还要用到拿取profile，所以把它提取成renderProfile来赋值。时间转化成生日需要实现formatDate

提交时，是服务器告诉状态，完成也是服务器告诉，所以之前onSubmit的代码就不要了。

先作假在submit服务中创建goroutine来修改认证状态为完成。在服务端提交完后去轮询，当得到状态不为pending时去停止轮询。轮询使用setInterval。
...
由于性别和出生年月改变时有事件处理函数，而驾驶证号和名字没有事件处理函数，审查完后驾驶证号和名字没有变化。在value=""之前加一个model:即可

###### 身份认证实现

现在就可以实现ACL层profile的认证服务。定义一个Fetcher接口里面包含GetProfile，这个参数是和service中的GetProfile的参数是一致的。Manager去包含Fetcher,verify中直接去使用fetch的getprofile,得到profile后，判断p.IndentityStatus如果不是verified就返回nil,这里就是侵入的概念，ACL层就是为了这里加的。
